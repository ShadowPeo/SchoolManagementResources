...
$tableName = "Students" # Adjust table name as needed

$adUsers = Get-ADUser -SearchBase $studentSearchBase -Filter * -Properties otherMailbox
    ...
    if ($user.otherMailbox) {
        ...
        if (-not [string]::IsNullOrWhiteSpace($username)) {
            try {
                ...
                $existingRecord = Invoke-Sqlcmd -ConnectionString $connectionString -Query "SELECT * FROM $tableName WHERE SIS_ID = '$($user.SamAccountName)'"
                Write-Host "TESTING"
                $query = @"
                IF EXISTS (
